...
//		$target_url="http://www.imdb.com/title/tt4383594";
	...
	private function scrapeMovieInfo($imdbUrl, $getExtraInfo = true)
	{
		...
		if($getExtraInfo == true) {
			...
			$imdb_html = new simple_html_dom();
$imdb_html->load_file($imdbUrl);
foreach($imdb_html->find('div[class=inline canwrap]') as $link)
{
   foreach($link->find('p') as $p) 
       {
             $arr['storyline2'] =  $p->plaintext;// do something...
       }
}
