...
include("online_users.php");
 ...
 if(isset($_GET["id"]))
        ...
        $select_views="select views from wallpapers where 
        ...
        $update_view_by_id="update wallpapers set views='$fetched_views' where id=".$wallpaper_id."";
...
<?php
	...
	<!---- End Advertisement ---->
        ...
        <div class="catalog-album-holder">
			<h2><a href="wallpapers.php">Wallpapers</a> &#155; 
			<?php
			
			$select_category_wallpaper="select wc.name 'wallpaper_category_name' 
		...
		$result_category_wallpaper = mysqli_query($conn,$select_category_wallpaper);
		
		$count_category_wallpaper = mysqli_num_rows($result_category_wallpaper);
		
		if($count_category_wallpaper > 0)
		{
			while($row_category_wallpaper = mysqli_fetch_array($result_category_wallpaper))
			{
				echo $row_category_wallpaper["wallpaper_category_name"];
			}
		}
		else
		{
		...
		?>
			...
			<div class="box clearfix inline-block">
			    <input type="text" name="search" placeholder="Just Wallpapers Search Engine..." class="email"/>
        	...
        	<div class="covers">
            	<ul class="single-album">
                	<li>
                        ...
                        $select_wallpaper_by_id="select * from wallpapers where
                   ...
                   $result_wallpaper_by_id=   mysqli_query($conn,$select_wallpaper_by_id);
                   
                   
                   $count_wallpaper_by_id=mysqli_num_rows($result_wallpaper_by_id);
                   
                   if($count_wallpaper_by_id > 0)
                   {
                    while($row_wallpaper_by_id =mysqli_fetch_array($result_wallpaper_by_id))
                    {
                    ?>
                    
                    <div class="single-album-cover">
                        	<img src="<?php echo $row_wallpaper_by_id["image_url"]; ?>" alt="<?php echo $row_wallpaper_by_id["name"]; ?>">
                        </div>
                    ...
                    <div class="single-album-info">
                        	<h1><?php echo $row_wallpaper_by_id["name"]; ?></h1>
                            <div class="single-album-infofull">
							<div class="single-album-info-3">
							                 	        </div>
                            <div class="single-album-info-4">
                            	
								<div><span>Views     </span>: <b><?php echo $row_wallpaper_by_id["views"]; ?></b></div>
								<div><span>Downloads     </span>: <b><?php echo $row_wallpaper_by_id["downloads"]; ?></b></div>
                            </div>
							</div>
                        </div>
                    ...
                    <div class="single-album-info">
						...
						<tr>
							...
							<td width="10"> : </td>
							<td class="movie_perview"><?php echo $row_wallpaper_by_id["description"]; ?></td>
						</tr>
                     ...
                     <tr>
							...
							<td width="10"> : </td>
							<td><?php echo $row_wallpaper_by_id["create_date"]; ?></td>
						</tr>
            	...
            	<ul class="single-album">
        <img src="<?php echo $row_wallpaper_by_id["image_url"]; ?>" style="width: auto;height:auto; ">
				</ul>
            	...
            	<div class="covers-holder">
                    <div class="list_carousel">
                        <div class="carousel-title-nav">
                            <h2>Related: Wallpapers <a href="wallpapers.php"><i class="fa fa-arrow-circle-o-right" title="More&#133;"></i></a></h2>
                    ...
                    <ul id="covers-trigger-1" >
                        ...
                        //select the data
                        $select_wallpapers="select * from wallpapers 
                         ...
                         if($count_wallpapers > 0)
                    {
                            ...
                            {
                                $fetched_category_id=$row_wallpapers["fk_wallpaper_category_id"];
                            }
                          
                          
                          //now select the related gener upcoming movies
                          $select_related_wallpaper="
                                                select wc.name 'wallpaper_category_name',w.id,w.name,w.description,w.views,w.downloads,w.image_url,w.create_date
												from wallpapers w,wallpaper_categories wc
												where 
												wc.id=w.fk_wallpaper_category_id
												and
												w.fk_wallpaper_category_id=".$fetched_category_id."
												and
												w.id not in (".$wallpaper_id.")
												and
												w.is_deleted=0
												and
												wc.is_deleted=0
                       order by w.id desc limit 10";
                        ...
                        $result_related_wallpaper=mysqli_query($conn,$select_related_wallpaper);
                        
                        
                        $count_related_wallpaper=mysqli_num_rows($result_related_wallpaper);
                        
                        if($count_related_wallpaper > 0)
                        
                        
                        {
                          while($row_related_wallpaper=mysqli_fetch_array($result_related_wallpaper))
                          {
                            ?>
                             <li><a  href="wallpaperpreview.php?id=<?php echo $row_related_wallpaper["id"]; ?>"><img class="movie_image" src="<?php echo $row_related_wallpaper["image_url"];  ?>" alt="<?php echo $row_related_wallpaper["name"];  ?>"><div></div></a>
						<a href="wallpaperpreview.php?id=<?php echo $row_related_wallpaper["id"]; ?>"><span class="name" title="<?php echo $row_related_wallpaper["name"]; ?>"><?php echo $row_related_wallpaper["name"]; ?></span></a>
						<p class="categoryname2"><a href="#" class="wrap-space"><?php echo $row_related_wallpaper["wallpaper_category_name"]; ?></a></p>
						
						<div class="VideoValue value"><p>View: <?php echo $row_related_wallpaper["views"]; ?></p>
						
						</div>
						<p class="value">Added :  <?php echo $row_related_wallpaper["create_date"]; ?></p>
						</li> 
                            <?php
                          }
                        }
